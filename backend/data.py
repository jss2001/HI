...
SECTORS = [
    {
        ...
        "change": 3.42,
        "price": 1842.55,
        "summary": "AI 반도체 수요 급증, 빅테크 실적 호조",
    },
    {
        "id": "semiconductor",
        "name": "반도체",
        ...
        "change": 2.18,
        "price": 4213.00,
        "summary": "HBM 공급 확대, 고대역폭 메모리 가격 강세",
    },
    {
        "id": "battery",
        "name": "2차전지",
    ...
    },
    {
        ...
        "change": -1.12,
        "price": 1675.80,
        "summary": "글로벌 판매 둔화 우려 지속",
    },
]


def _generate_chart(base_price: float, change_pct: float, points: int = 40):
    series = []
    start = base_price / (1 + change_pct / 100)
    for i in range(points):
        t = i / (points - 1)
        wave = math.sin(t * math.pi * 2.2) * (base_price * 0.004)
        drift = (base_price - start) * t
        value = round(start + drift + wave, 2)
        series.append(value)
    return series
...
SECTOR_DETAILS = {
    s["id"]: {
        **s,
        ...
        "chart": _generate_chart(s["price"], s["change"]),
        "comments": [
            {
                "id": f"{s['id']}-c1",
                "user": "투자자_민준",
                "time": "방금 전",
                "text": f"{s['name']} 섹터 오늘 흐름 좋네요. 외인 순매수 들어오는 듯.",
                "likes": 24,
            },
            {
                "id": f"{s['id']}-c2",
                "user": "차트분석가",
                "time": "5분 전",
                "text": "단기 저항선 돌파 시도 중. 거래량 증가 확인됨.",
                "likes": 17,
            },
            {
                "id": f"{s['id']}-c3",
                "user": "장기투자러",
                "time": "12분 전",
                "text": "장기적으로 우상향 그림 좋아 보입니다. 분할매수 중.",
                "likes": 9,
            },
            {
                "id": f"{s['id']}-c4",
